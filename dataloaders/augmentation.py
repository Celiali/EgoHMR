...
def get_example(img_path: str, center_x: float, center_y: float,
    ...
    """
    perform augmentation for image, 2d keypoints, 3d keypoints, scene vertices, smpl params
    """
    ############################### load image
    ...
    ############################### get 2d keypoints in aug cropped img
    ...
    keypoints_2d_vis_mask = (keypoints_2d_crop[:, -1]>0).copy()  # [25]
    ...
    keypoints_2d_vis_mask = keypoints_2d_vis_mask * temp
    ...
    # # for debug: uncomment if need to return aug orig img in data loader(for visualization)
    # rotated_cvimg = cv2.warpAffine(cvimg, M, (1920, 1080))

    ############################### get augemented 3d joints / scene-vertices in: auge crop cam and auge full cam
    ...
    # scene_pcd.points = o3d.utility.Vector3dVector(scene_pcd_verts_full_auge)
    # # scene_pcd.colors = o3d.utility.Vector3dVector(scene_pcd_colors)
    # keypoints_pcd = o3d.geometry.PointCloud()
    # keypoints_pcd.points = o3d.utility.Vector3dVector(keypoints_3d_full_auge)
    #
    # body_params_dict_new = {}
    # body_params_dict_new['global_orient'] = smpl_params['global_orient']
    # body_params_dict_new['transl'] = smpl_params['transl']
    ...
    #
    # o3d.visualization.draw_geometries([scene_pcd, mesh_frame, keypoints_pcd, gt_body_o3d])

    rotated_img = 0
    rotated_cvimg = 0
    # # for debug: uncomment if need to return aug orig img in data loader(for visualization)
    # rotated_cvimg = rotated_cvimg[:, :, ::-1]  # [224, 224, 3] BGR-->RGB
    # rotated_img = convert_cvimg_to_tensor(rotated_cvimg)
    # # apply RGB normalization
    # for n_c in range(img_channels):
    #     rotated_img[n_c, :, :] = np.clip(rotated_img[n_c, :, :] * color_scale[n_c], 0, 255)
    #     if mean is not None and std is not None:
    #         rotated_img[n_c, :, :] = (rotated_img[n_c, :, :] - mean[n_c]) / std[n_c]


    ############################### get 2d keypoints for augemented orig size img
    ...
    # output_img = pil_img.fromarray((rotated_cvimg).astype(np.uint8))
    # draw = ImageDraw.Draw(output_img)
    # for k in range(len(keypoints_2d_aug_orig)):
    #     draw.ellipse((keypoints_2d_aug_orig[k][0] - 4, keypoints_2d_aug_orig[k][1] - 4,
    #                   keypoints_2d_aug_orig[k][0] + 4, keypoints_2d_aug_orig[k][1] + 4), fill=(0, 255, 0, 0))
    # output_img.show()
    # #############
    ...
    return img_patch, keypoints_2d_crop, keypoints_2d_vis_mask, keypoints_2d_aug_orig, \
